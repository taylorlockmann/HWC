...
myspecies_coords <- as.data.frame(results$gbif$data$Panthera_leo) %>% # using GBIF data
  ...
  filter(!basisOfRecord %in% c("FOSSIL_SPECIMEN", "PRESERVED_SPECIMEN"),
        ...
        eventDate <= "2000-12-31" & eventDate >= "1970-01-01")
```
...
output <- spThin::thin(occs, 'latitude', 'longitude', 'name', 
                       ...
                       reps = 5, # default on Wallace is 100, but run time is long, so a low number is good for testing
...
#      write_csv(occs_wallace, "H:/My Drive/HWC/R_files/R_output_data/occurrence_points/bioclim_points_thinned.csv")
```
...
library(Hmisc)
occ_extract_final_bio_sub_matrix <- as.matrix(occ_extract_final_bio_sub)
rcorr(occ_extract_final_bio_sub_matrix, type="pearson") # type can be pearson or spearman

# d2 <- occ_extract_final_bio_sub %>% 
#   as.matrix %>%
#   cor %>%
#   as.data.frame %>%
